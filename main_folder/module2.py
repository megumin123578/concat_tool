...
def insert_logo(input_video, logo_path, output_video, position="bottom-left", logo_width=150):
    positions = {
        "top-left": "10:10",
        "top-right": "W-w-10:10",
    ...
    if position not in positions:
        raise ValueError(f"Vị trí logo không hợp lệ: {position}")

    overlay = positions[position]

    if logo_width:
        filter_complex = f"[1:v]scale={logo_width}:-1[logo];[0:v][logo]overlay={overlay}"
    else:
    ...
    try:
        subprocess.run(command, check=True)
        print(f"Đã chèn logo: {output_video}")
    except subprocess.CalledProcessError as e:
        print(f"Lỗi khi chèn logo vào {input_video}: {e}")



# auto_concat(input_videos, output_path)
